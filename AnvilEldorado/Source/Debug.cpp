...
namespace AnvilEldorado
{
	...
	{
		auto srcFilter = Debug::Instance()->VarMemcpySrc->ValueInt;
		auto dstFilter = Debug::Instance()->VarMemcpyDst->ValueInt;
		...
		if (srcFilter >= srcAddress && srcFilter < srcAddress + len)
		{
			...
			// TODO: proper reset
			Debug::Instance()->VarMemcpySrc->ValueInt = 0;
		}
		...
		if (dstFilter >= dstAddress && dstFilter < dstAddress + len)
		{
			...
			// TODO: proper reset
			Debug::Instance()->VarMemcpyDst->ValueInt = 0;
		}
	...
	{
		auto src = Debug::Instance()->VarMemcpySrc->ValueInt;
	...
	{
		auto dst = Debug::Instance()->VarMemcpyDst->ValueInt;
	...
	{
		uint32_t dstFilter = Debug::Instance()->VarMemsetDst->ValueInt;
		uint32_t dstAddress = (uint32_t)dst;
		...
		if (dstFilter >= dstAddress && dstFilter < dstAddress + len)
		{
			...
			// TODO: proper reset
			Debug::Instance()->VarMemsetDst->ValueInt = 0;
		}
	...
	{
		auto dst = Debug::Instance()->VarMemsetDst->ValueInt;
	...
	Debug::Debug()
		: Module("Debug")
	{
		VarMemcpySrc = AddVariableInt(
			"MemcpySrc", "memcpy_src",
			"Allows breakpointing memcpy based on specified source address filter.",
			CommandFlags::Hidden, 0, MemcpySrcFilterUpdate);

		VarMemcpyDst = AddVariableInt(
			"MemcpyDst", "memcpy_dst",
			"Allows breakpointing memcpy based on specified destination address filter.",
			CommandFlags::Hidden, 0, MemcpyDstFilterUpdate);

		VarMemsetDst = AddVariableInt(
			"MemsetDst", "memset_dst",
			"Allows breakpointing memset based on specified destination address filter.",
			CommandFlags::Hidden, 0, MemsetDstFilterUpdate);
	}
	...
	bool Debug::Init()
	{
		...
		return Hook(0x7EF260, Debug_MemcpyHook).Apply()
