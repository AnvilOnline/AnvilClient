...
namespace Blam
{
	...
	struct DataIteratorBase
	{
		...
		explicit DataIteratorBase(const DataArrayBase *data) : Array(data), CurrentDatumIndex(Data::DatumIndex::Null), CurrentIndex(-1) { }
		...
		Data::DatumIndex CurrentDatumIndex; // The datum index of the current datum
	...
	// TDatum MUST inherit from the Data::DatumBase struct.
	template<class TDatum>
	struct DataArray : DataArrayBase
	{
		static_assert(std::is_base_of<Data::DatumBase, TDatum>::value, "TDatum must inherit from Data::DatumBase");

		// Gets a pointer to the datum corresponding to a datum index.
		// Returns null if the datum index does not match a valid datum.
		TDatum* Get(Data::DatumIndex index) const { return static_cast<TDatum*>(DataArrayBase::Get(index)); }

		// Gets a reference to the datum corresponding to a datum index.
		// The datum index is NOT checked for validity and this will always succeed.
		// Use Get() if you need validity checking.
		TDatum& operator[](Data::DatumIndex index) const { return *static_cast<TDatum*>(GetAddress(index)); }
	...
	static_assert(sizeof(DataArray<Data::DatumBase>) == sizeof(DataArrayBase), "Invalid DataArray size");
	...
	static_assert(sizeof(DataIterator<Data::DatumBase>) == sizeof(DataIteratorBase), "Invalid DataIterator size");
	...
	static_assert(sizeof(ConstDataIterator<Data::DatumBase>) == sizeof(DataIteratorBase), "Invalid ConstDataIterator size");
}